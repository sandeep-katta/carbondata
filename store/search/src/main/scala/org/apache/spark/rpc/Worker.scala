...
import org.apache.spark.util.{CarbonReflectionUtils, ThreadUtils}
...
object Worker {
  ...
  private def registerToMaster(masterHostAddress: String, masterPort: Int): String = {
    ...
    val conf = new SparkConf()
    val config = CarbonReflectionUtils.getRpcEnvConfig(conf, "registry-client", masterHostAddress, "", masterPort,
